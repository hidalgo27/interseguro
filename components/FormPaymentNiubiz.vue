...
export default {
  ...
  data () {
    return {
      ...
      botonTexto: 'Pagar S/' + this.$store.state.common.montoPagar, // el texto del boton pagar
    }
  },
  mounted () {
    let self = this
    ...
    this.$store.dispatch('payment/getSessionKey', self.session).then(respuesta => {
      ...
      if (respuesta.data.code === '01') {
        ...
        self.niubizConfig.callbackurl = 'https://test.interseguro.pe/payment-api/api/v2/token?protocol=https&host=test.interseguro.pe/'+'&path='+ 'vehicular/cotiza/como-pagar'
        ...
        setTimeout(function () {
          ...
          if (document.querySelectorAll("#inputFechaExpiracion")[0].children[0].children.length < 1) {
            ...
            self.$nuxt.$emit('hide-loader')
            self.$nuxt.$emit('show-payment',{
              payment: 'CULQI'
            })
          }
        }, 10500)
        self.$nuxt.$emit('show-loader',false)
      } else {
        self.niubizConfig.sessionkey = ''
        // ocultar el formulario de niubiz
        self.$nuxt.$emit('show-payment',  {
