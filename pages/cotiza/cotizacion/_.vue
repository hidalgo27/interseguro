    ...
    <b-modal id="leaveQuote" class="leaveModal" size="lg" static centered hide-footer hide-header>
      <b-container>
        ...
        <b-row class="text-center">
          ...
          <b-col cols="12" class="mb-3">
            <h3>Protege tu {{this.itemElegido.brand}} con los mejores <br> precios, desde ${{this.listaBasica.policy.monthly}}. </h3>
          </b-col>
    ...
    <b-modal id="leaveQuote2" class="leaveModal" size="lg" static centered hide-footer hide-header>
      <b-container>
        ...
        <b-row class="text-center">
          ...
          <b-col cols="12" class="mb-3">
            <h3>Elige un plan para tu {{this.itemElegido.brand}} <br> desde ${{this.listaBasica.policy.monthly}} y maneja seguro</h3>
          </b-col>
          <b-col cols="12" class="mb-2">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getCotizacion() {
          this.$store.dispatch('common/getCotizacion', this.itemElegido).then((res) => {
            ...
            this.$store.commit('common/setCurrent', this.listCotizacion.vehicle.current)
            this.remarketingv2(this.$store.state.common.email, 1)
          })
        // }
    ...
    mounted: function () {
      ...
      var parametros = this.$route.params.pathMatch;
      var parametrosArreglo = parametros.split(["/"])

      this.objectVehicle.brandId = parametrosArreglo[0];
      this.objectVehicle.modelId = parametrosArreglo[1];
      this.objectVehicle.modelYear = parametrosArreglo[2];
      this.$store.commit('common/setDocumentoLocal', parametrosArreglo[3]);
      this.$store.commit('common/setPlateNumber', parametrosArreglo[4]);
      this.item.plateNumber = parametrosArreglo[4]
      if(parametrosArreglo.length == 1){
      }else if(parametrosArreglo.length == 5){
        this.$store.commit('common/setFlujoDetectado', 'soat')
        ...
        this.$store.dispatch('common/getSoat', 
          {
            brandId: this.objectVehicle.brandId,
            modelId: this.objectVehicle.modelId,
            modelYear: this.objectVehicle.modelYear,
            documentNumber: parseInt(parametrosArreglo[3],0),
            plateNumber: parametrosArreglo[4],
            source: 1
          }).then((res) => {
            this.$store.commit('common/setEmail', res.data.body.email.trim().replace(/ /g, ""))
            this.$store.dispatch('common/getVehicle', 
            {
              plateNumber: parametrosArreglo[4],
              ...
              if (res.data.code == 0) {
                this.objectVehicle = res.data.body
                  ...
                  /* this.$nuxt.$router.push({path: "/pageNotFound"}) */
                }
            })
        })
        .catch((res)=>{
